...
impl<R: Read, const S: usize> BufReader<R, S> {
    /// Creates a new `BufReader<R, S>` with a default buffer capacity. The default is currently 8
    /// KB, but may change in the future.
    ///
    /// # Examples
    ///
    /// ```ignore
...
impl<W: Write, const S: usize> Write for LineWriterShim<'_, W, S> {
