    ...
    loop {
        if g.len == g.buf.len() {
            unsafe {
                ...
                //
                //   - This creates a (mut) reference to a slice of _uninitialized_ integers, which
        ...
        match r.read(buf) {
            ...
            Ok(n) => {
                // We can't allow bogus values from read. If it is too large, the returned vec could
                // have its length set past its capacity, or if it overflows the vec
                // could be shortened which could create an invalid string if this
